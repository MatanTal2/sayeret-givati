...
23. ~~**Mobile zoom-in on text-input focus inside modal sticks after modal close (UX)**~~
    - **FIXED (2026-05-14 on `fix/mobile-input-zoom-bug-23`):** picked mitigation **(a)** from the bug brief — prevent the zoom by guaranteeing `font-size ≥ 16px` on every focusable form control on touch viewports. iOS Safari / Android Chrome only auto-zoom when the focused control's computed font-size is `< 16px`; once that's denied, the browser never enters the zoomed state, so there's nothing to "restore" on blur.
    - **Implementation** (`src/app/globals.css`): added a `@media (pointer: coarse)` block that pins `input` (excluding checkbox/radio/file/range/color — none of those trigger zoom and several would be visually distorted by the size bump), `textarea`, and `select` to `font-size: 16px`. Desktop (`pointer: fine`) keeps the compact Tailwind `text-sm` (14px) sizing on form controls; only touch viewports get the floor.
    - **Why (a) over (b):** (b) would have meant toggling `<meta name="viewport" maximum-scale=1>` on every modal open/close — a global, app-wide side-effect for what is fundamentally a per-control problem, and one with real a11y cost (blocks pinch-zoom for users who need it). (a) is purely additive CSS, scoped to the actual trigger condition (`pointer: coarse`), and survives any new modal/wizard without per-component wiring.
    - **Scope:** applies globally to every `input` / `textarea` / `select` on the site, not just the ones inside `.modal-overlay`. Modals were the loudest reproducer but the same auto-zoom hits *any* text input on small viewports (profile form, search bars, etc.) — bumping all of them is the correct fix.
...
26. ~~**Phone book "refresh" doesn't reflect upstream deletions**~~ *(phone book)*
    - **FIXED (2026-05-14 on `fix/bug-batch-2026-05-14-pt3`):** root cause was the server-side delete leg, not the client cache. The client refresh path already invalidated `phoneBookCache` and force-fetched (`usePhoneBook.refresh` calls `clearPhoneBookCache()` before `forceFullFetch`). But `serverDeletePersonnel` (`src/lib/db/server/authorizedPersonnelService.ts`) only deleted the `authorized_personnel/{hash}` doc and left the downstream `phoneBook/{hash}` row in place, so every fresh fetch kept returning it.
