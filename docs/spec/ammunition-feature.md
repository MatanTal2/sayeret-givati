...
- **Resume at:** **Phase 6 — Manager-triggered report requests**. Mirror `reportRequestService.ts` for ammunition: `serverCreateAmmunitionReportRequest` writes a doc with scope ∈ {INDIVIDUAL, TEAM, ALL}, materializes `targetUserIds` server-side for TEAM/ALL, blasts `ammo_report_requested` notifications. `serverFulfillAmmunitionReportRequest` patches `fulfillmentByUser[reporterId]`. Wire `serverSubmitAmmunitionReport` to also patch fulfillment when `reportRequestId` is set in the same transaction. New `src/app/api/ammunition-report-requests/route.ts` (POST + PATCH cancel). Hook `useAmmunitionReportRequests`. Replace placeholder `AmmunitionRequestsSection.tsx` with manager UI (scope picker + optional template list + due date; per-request fulfillment progress). `ReportUsageForm` to consume a `requestId` deep-link prop and pre-fill template list. NotificationItem already routes `ammo_report_requested` → `/ammunition` (Phase 4); extend with `?requestId=...`.
