...
- **Resume at:** **Phase 5 — Manager dashboard subsection**. Replace placeholder `AmmunitionReportsSection.tsx` with the real Reports subsection inside the existing single Ammunition tab. Filters: date range + subcategory + template + team + reporter. Sortable table (mirror `AuditLogsTab`). Columns: timestamp, reporter, team, template (with security-level badge), consumed (formatted per tracking mode), reason. "Export CSV" button — `src/lib/ammunition/csvExport.ts` with Hebrew columns + UTF-8 BOM for Excel.
