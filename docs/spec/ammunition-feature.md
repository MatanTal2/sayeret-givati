...
- **Phase 4 status:** COMPLETE. `src/lib/db/server/ammunitionReportsService.ts` runs `serverSubmitAmmunitionReport` in a single transaction: write `ammunitionReports`, decrement `ammunitionInventory` for BRUCE/LOOSE_COUNT (rejects over-consumption) or flip `ammunition` SERIAL items to CONSUMED (asserts each item's holder is the reporter). After the txn: action-log entry + batched notifications to TL(s) (`teamId == reporter.teamId AND userType == TEAM_LEADER`) ∪ `systemConfig/main.ammoNotificationRecipientUserId`, deduped, reporter excluded. Client: `src/lib/ammunition/reportsService.ts`, `useAmmunitionReports`, `/api/ammunition-reports` (POST + GET with date/team/reporter/template filters). UI: `ReportUsageForm` (template picker filtered to reporter's holdings + per-tracking-mode inputs + datetime-local usedAt). Wired a "דווח על שימוש" CTA into `/ammunition`. NotificationItem deep-link router routes `ammo_report_submitted`/`ammo_report_requested` → `/ammunition`. Test: `src/lib/__tests__/ammunitionReportsService.test.ts` — 11 passing. Lint + clean `npm run build` green.
- **Phase 5 status:** COMPLETE. `src/lib/ammunition/csvExport.ts` (BOM + CRLF + Hebrew column headers, per-tracking-mode `consumed` formatter). Real `AmmunitionReportsSection.tsx` replaces the placeholder — date-range + subcategory + template + team + reporter filters, four sortable columns (usedAt/reporterName/teamId/templateName), security-level badge column, "ייצוא CSV" downloads the filtered + sorted view. Test: `src/lib/__tests__/ammunitionCsvExport.test.ts` — 3 passing (BOM + header structure, comma/quote/newline escaping, SERIAL serials joined). Lint + clean build green.
- **Phase 6 status:** COMPLETE. `src/lib/db/server/ammunitionReportRequestService.ts` (create/cancel/list + `serverPatchFulfillment` invoked by the report submit when `reportRequestId` is set). TEAM/ALL targets materialized server-side from active `users`. Notification fan-out passes `relatedEquipmentDocId = request.id` so deep links can carry it. `/api/ammunition-report-requests` (POST + PATCH cancel + GET). Hook `useAmmunitionReportRequests`. Real `AmmunitionRequestsSection.tsx` replaces the placeholder — scope picker (INDIVIDUAL → user typeahead, TEAM → team id, ALL → none), template multi-select, due date, note. List shows scope + status badge + fulfilled/total + creator. Cancel button on OPEN requests. `ReportUsageForm` accepts `reportRequestId` + `restrictTemplateIds` and threads them into the submit payload. `/ammunition` page reads `?requestId=` from the URL and auto-opens the form pre-filled. NotificationItem routes `ammo_report_requested` → `/ammunition?requestId=<id>`. Added `targetTeamId` + `note` fields to `AmmunitionReportRequest`. Test: `src/lib/__tests__/ammunitionReportRequestService.test.ts` — 9 passing. Lint + clean build green.
- **Resume at:** **Phase 7 — Training planner stub**. Create `src/app/ammunition/training/page.tsx` with a "בקרוב" placeholder + a one-paragraph future-spec note in this file. Modify `/ammunition/page.tsx` to add a small nav link to `/ammunition/training`. Then **Phase 8 — Firestore rules + final docs sweep** (lock down access for all 6 ammunition collections + `systemConfig/main`; refresh `docs/firebase-operations.md` + `docs/duplications.md`).
