...
- **Resume at:** **Phase 3 — Inventory CRUD + read views**. Server `ammunitionInventoryService` (upsert BRUCE/LOOSE_COUNT stock per holder, create/update/delete SERIAL items, tiered permission checks per call), client `inventoryService`, `useAmmunitionInventory`, `/api/ammunition-inventory` root + `[id]`. Replace placeholder `AmmunitionInventorySection.tsx` with full CRUD UI. New shared `AmmunitionInventoryView` (used in management + the user `/ammunition` page) and `AddInventoryModal`. New page `src/app/ammunition/page.tsx` (own + team inventory). Modify `/equipment/page.tsx` to render a "תחמושת אישית" section reading `ammunitionInventory` filtered to USER-allocated holdings of the current user. Add ammo card on home page.
