...
- **Resume at:** **Phase 2 — Ammunition Templates CRUD + Management tab shell**. Seed canonical templates (`src/data/ammunitionTemplates.ts`), fixed subcategory list (`src/lib/ammunition/subcategories.ts`), server `ammunitionTemplatesService`, client `templatesService`, `/api/ammunition-templates` routes, `useAmmunitionTemplates`, `AmmunitionTemplateForm`, the new `AmmunitionTab` with internal sub-tabs, and the `AmmunitionTemplatesSection` (other three sub-sections rendered as "coming next phase" placeholders). Wire into `useManagementTabs` + `TabContentRenderer`.
