...
- `approvedRole` is now typed as `UserRole` (bug #7 fix). `addAuthorizedPersonnel` honors `formData.approvedRole` (defaulting to `UserRole.SOLDIER`); `addAuthorizedPersonnelBulk` honors `person.approvedRole` per row with the same default. `updateAuthorizedPersonnel` accepts an `approvedRole` field, validates it against `USER_ROLE_VALUES`, and propagates the change to the synced `users` doc via `shouldSync` when the soldier is registered.
