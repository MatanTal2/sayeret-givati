...
interface RejectExchangeModalProps {
  equipment: Equipment;
  onClose: () => void;
  onSubmit: (reason: string) => Promise<{ success: boolean; error?: string }>;
}

export default function RejectExchangeModal({ equipment, onClose, onSubmit }: RejectExchangeModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        <header className="flex items-start justify-between p-5 border-b border-neutral-200">
          <div>
            <h2 className="text-lg font-semibold text-neutral-900">{labels.REJECT_TITLE}</h2>
        ...
        <div className="p-5 space-y-4">
          <div className="rounded-lg p-3 text-sm bg-danger-50 text-danger-800 border border-danger-200">
            {labels.REJECT_DESCRIPTION}
          </div>
        ...
        <footer className="flex items-center justify-end gap-2 p-5 border-t border-neutral-200">
          ...
          <button type="button" onClick={handleSubmit} className="btn-danger" disabled={submitting}>
            {submitting ? '...' : labels.SUBMIT_REJECT}
          </button>
        </footer>
      </div>
    </div>
